...
class TestAdminMSDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        conn_params = {
            ...
            self.mscp.DB_ROOT_USER_PASSWORD: self.test_cfg["arangodb"][
        ...
        self.admin = ArangoPipeAdmin(
            reuse_connection=False, config=conn_config, persist_conn=False
        )
    ...
    def test_using_deleted_database(self):
        ...
        try:
            ...
            conn_config = ArangoPipeConfig()
            conn_params = {
                mscp.DB_SERVICE_HOST: self.test_cfg["arangodb"][
                    self.mscp.DB_SERVICE_HOST
                ],
                mscp.DB_SERVICE_END_POINT: self.test_cfg["arangodb"][
                    self.mscp.DB_SERVICE_END_POINT
                ],
                mscp.DB_SERVICE_NAME: self.test_cfg["arangodb"][
                    self.mscp.DB_SERVICE_NAME
                ],
                mscp.DB_SERVICE_PORT: self.test_cfg["arangodb"][
                    self.mscp.DB_SERVICE_PORT
                ],
                mscp.DB_CONN_PROTOCOL: self.test_cfg["arangodb"][
                    self.mscp.DB_CONN_PROTOCOL
                ],
            }
            conn_config = conn_config.create_connection_config(conn_params)
    ...
    def delete_users(self):
        ...
        host_connection = (
            self.test_cfg["arangodb"][self.mscp.DB_CONN_PROTOCOL]
            + "://"
    ...
    def test_delete_database(self):
        ...
        try:
            ...
            traceback.print_exc(file=sys.stdout)
            print("-" * 60)
            self.assertTrue(err_raised, "Exception raised while deleting database")
        self.assertFalse(err_raised)
